...
console.log("🔥 WEBHOOK HIT");
  ...
  try {
    ...
    // ── Recording ready ───────────────────────────────────────────────────────
   // Inside app/api/webhooks/stream/route.js

if (eventType === "call.recording_ready") {
  // Catch the URL regardless of how it's nested in the payload
  const recordingUrl = body.call_recording?.url || body.call?.recordings?.[0]?.url;

  if (!recordingUrl) {
    console.log("[stream-webhook] No URL found in payload yet.");
    return Response.json({ ok: true });
  }

  // UPDATE: This ensures the URL is saved and the status changes so the UI shows it
  await db.booking.update({
    where: { id: booking.id },
    data: { 
      recordingUrl: recordingUrl,
      status: "COMPLETED" // Changes 'SCHEDULED' to 'COMPLETED' automatically
    },
  });

  console.log(`✅ Video saved for booking ${booking.id}`);
  return Response.json({ ok: true });
}
    // ── Transcription ready ───────────────────────────────────────────────────
